...
const turnobjectToArray = pieces => Object.keys(pieces).map(pckey => {
...
const isSamePosition = (fromFireBase, fromThisPieces) =>
  (fromFireBase.x === fromThisPieces.position.x) &&
  (fromFireBase.y === fromThisPieces.position.y)
...
export default class extends Phaser.State {
  init () {
    ...
    firebase.database().ref(`lobbiesTest/${this.game.lobby}/game`).set({currentPlayer: 'red'})
    firebase.database().ref(`lobbiesTest/${this.game.lobby}`).on('value', snapshot => {
      firebaseState = snapshot.toJSON()
      // console.log('this.pieces', this.pieces)
      // console.log('firebaseState', firebaseState)
      if (this.pieces) {
        console.log('firebaseState.pieces', firebaseState.pieces)
        Object.keys(firebaseState.pieces).forEach(pieceId => {
          const thispiecesId = firebaseState.pieces[pieceId].id
          console.log('this.pieces', this.pieces)
          console.log('pieceId', pieceId)
          console.log('this.pieces[pieceId]', this.pieces[thispiecesId])
          if (!isSamePosition(firebaseState.pieces[pieceId], this.pieces[thispiecesId])) {
            this.pieces[thispiecesId].position.x = firebaseState.pieces[pieceId].x
            this.pieces[thispiecesId].position.y = firebaseState.pieces[pieceId].y
          }
        })
      }
    })
  ...
  create () {
    ...
    firebase.database().ref(`lobbiesTest/${this.game.lobby}/pieces`).set(turnobjectToArray(this.pieces))
  }

  togglePlayer () {
    this.currentPlayer = this.currentPlayer === 'red' ? 'blue' : 'red'
    firebase.database().ref(`lobbiesTest/${this.game.lobby}/game`).set({currentPlayer: this.currentPlayer})

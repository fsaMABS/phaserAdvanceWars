...
const MenuState = {
  ...
  create: function () {
    ...
    this.title = this.add.text(this.game.world.centerX, 210, 'Advance Wars', {
      font: '64pt Arial',
      fill: 'white',
      fontStyle: 'oblique',
      fontWeight: 'bold'
    ...
    this.onlineSelect = this.game.add.text(
      this.game.world.centerX,
      this.game.world.centerY,
      'Online Multiplayer',
      { font: '34pt Arial', fill: 'white', fontWeight: 'bold' }
    )
    this.onlineSelect.anchor.set(0.5)
    this.onlineSelect.inputEnabled = true
    this.onlineSelect.events.onInputDown.add(() => selectOnline(), this)
    this.onlineSelect.events.onInputOver.add(
      () => select(this.onlineSelect),
      this
    )
    this.onlineSelect.events.onInputOut.add(
      () => deSelect(this.onlineSelect),
      this
    )

    this.localSelect = this.game.add.text(
      this.game.world.centerX,
      this.game.world.centerY + 80,
      'Local Multiplayer',
      { font: '34pt Arial', fill: 'white', fontWeight: 'bold' }
    )
    this.localSelect.anchor.set(0.5)
    this.localSelect.inputEnabled = true
    this.localSelect.events.onInputDown.add(() => selectLocal(), this)
    this.localSelect.events.onInputOver.add(
      () => select(this.localSelect),
      this
    )
    this.localSelect.events.onInputOut.add(
      () => deSelect(this.localSelect),
      this
    )
